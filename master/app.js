...
function initNavbar() {
  ...
  const sections = document.querySelectorAll('section[data-bg], footer[data-bg]');
  if (!sections.length) {
    console.error('No sections with data-bg found. Add data-bg="dark" or data-bg="light" to each section.');
    return;
  }

  // Track which section is currently under the navbar
  const observerOptions = {
    rootMargin: '-40px 0px -85% 0px',  // detect section in the top 15% of viewport
      ...
      if (entry.isIntersecting) {
        ...
        if (bg !== currentBg) {
          ...
          if (bg === 'light') {
            ...
            console.log('Section under navbar is LIGHT — navbar switched to dark text');
          } else {
            navbar.classList.remove('navbar-on-light');
            console.log('Section under navbar is DARK — navbar restored to light text');
          }
        }
      }
    });
  ...
  function updateNavbarBg() {
    const navY = 42; // Y coordinate of floating navbar pill
    sections.forEach(sec => {
      const r = sec.getBoundingClientRect();
      if (r.top <= navY && r.bottom > navY) {
        const bg = sec.dataset.bg || 'dark';
        if (bg !== currentBg) {
          currentBg = bg;
          if (bg === 'light') {
            navbar.classList.add('navbar-on-light');
  ...
  console.log('Navbar bg-observer initialized. Watching', sections.length, 'sections.');
}
...
function initHeroDinoGame() {
  ...
  function spawnObstacle() {
    ...
    if (type === 'cactus-small') {
      obs = { type, x: W + 20, y: groundY - 20, w: 14, h: 20 };
    } else if (type === 'cactus-medium') {
      obs = { type, x: W + 20, y: groundY - 25, w: 14, h: 25 };
    } else if (type === 'cactus-large') {
      obs = { type, x: W + 20, y: groundY - 32, w: 20, h: 32 };
    } else {
      const heights = [groundY - 18, groundY - 40, groundY - 60];
      obs = { type, x: W + 20, y: heights[Math.floor(Math.random() * 3)], w: 32, h: 22, wing: 0 };
    }
    obstacles.push(obs);
  ...
  function spawnCloud() {
    clouds.push({ x: W + 20, y: 25 + Math.random() * 50, w: 36, h: 10 });
    nextCloudTime = frameCount + 80 + Math.floor(Math.random() * 120);
  }

  function checkCollision(o) {
    let dx = dino.x + 6, dy = dino.y - dino.h + 3;
    let dw = dino.w - 12, dh = dino.h - 6;
    if (dino.isDucking) { dh = DUCK_H; dy = dino.y - DUCK_H + 3; }
    return dx < o.x + o.w - 3 && dx + dw > o.x + 3 &&
           dy < o.y + o.h - 3 && dy + dh > o.y + 3;
  }
  ...
  function drawCactus(o) {
    ...
    if (o.type === 'cactus-small') {
      ctx.fillRect(o.x, o.y, 4, 20);
      ctx.fillRect(o.x - 3, o.y + 6, 3, 6);
      ctx.fillRect(o.x + 4, o.y + 4, 3, 8);
    } else if (o.type === 'cactus-medium') {
      ctx.fillRect(o.x, o.y, 4, 25);
      ctx.fillRect(o.x - 4, o.y + 8, 4, 8);
      ctx.fillRect(o.x + 4, o.y + 5, 3, 10);
    } else if (o.type === 'cactus-large') {
      ctx.fillRect(o.x, o.y, 5, 32);
      ctx.fillRect(o.x - 5, o.y + 10, 5, 11);
      ctx.fillRect(o.x + 5, o.y + 6, 5, 13);
      ctx.fillRect(o.x + 5, o.y + 24, 3, 8);
    }
  }
  ...
  function drawBird(o) {
    ...
    const flap = o.wing < 6;
    ctx.fillStyle = OBSTACLE_COLOR;
    ctx.fillRect(o.x, o.y, 16, 5);
    ctx.fillRect(o.x + 13, o.y - 2, 8, 7);
    ctx.fillStyle = BG; ctx.fillRect(o.x + 18, o.y, 2, 2);
    if (flap) {
      ctx.fillStyle = OBSTACLE_COLOR;
      ctx.fillRect(o.x + 3, o.y - 7, 11, 3);
      ctx.fillRect(o.x + 6, o.y - 10, 5, 3);
    } else {
      ctx.fillStyle = OBSTACLE_COLOR;
      ctx.fillRect(o.x + 3, o.y + 5, 11, 3);
      ctx.fillRect(o.x + 6, o.y + 8, 5, 3);
    }
  }
  ...
  function drawCloud(c) {
    ...
    ctx.fillRect(c.x + 5, c.y - 3, c.w - 10, 3);
    ctx.fillRect(c.x + 10, c.y - 6, c.w - 20, 3);
  }
  ...
  function drawGameOver() {
    ...
    ctx.fillText('G A M E   O V E R', W / 2, H / 2 - 15);
...
(function() {
  ...
  // ── Creator data ──
  const CREATORS = [
    { name: 'MortaL', realName: 'Naman Mathur', img: 'images-for-landing-page/mortal.png',
      instagram: { handle: 'mortal8bit', count: '4.5K' },
      youtube: { handle: '@Mortal', count: '100K' } },
    { name: 'Payal', realName: 'Payal Dhare', img: 'images-for-landing-page/payal.png',
      instagram: { handle: 'payalgaming', count: '1.8M' },
      youtube: { handle: '@PayalGaming', count: '2.6M' } },
    { name: 'Snax', realName: 'Raj Verma', img: 'images-for-landing-page/snax.png',
      instagram: { handle: 'snaxgaming', count: '1.3M' },
      youtube: { handle: '@SnaxGaming', count: '2.63M' } },
    { name: 'Jokerkihaveli', realName: 'Gulrez Khan', img: 'images-for-landing-page/joker.png',
      instagram: { handle: 'jokerkihaveli', count: '1.9M' },
      youtube: { handle: '@jokerkihavelii', count: '2.33M' } },
    { name: 'Goblin', realName: 'Abhijeet Andhare', img: 'images-for-landing-page/goblin.png',
      instagram: { handle: 'goblin8bit', count: '1.5M' },
      youtube: { handle: '@Goblin', count: '2.1M' } },
    { name: 'Mamba', realName: 'Sahil Soni', img: 'images-for-landing-page/mamba.png',
      instagram: { handle: 'mamba8bit', count: '800K' },
      youtube: { handle: '@Mamba', count: '1.2M' } },
    { name: 'Viper', realName: 'Akash Anand', img: 'images-for-landing-page/viper.png',
      instagram: { handle: 'viper8bit', count: '600K' },
      youtube: { handle: '@Viper', count: '900K' } },
    { name: 'Pahadi', realName: 'Ravi Singh', img: 'images-for-landing-page/pahadi.png',
      instagram: { handle: 'pahadi8bit', count: '4.5K' },
      youtube: { handle: '@Pahadi', count: '100K' } }
  ];
  ...
  function updateCaption() {
    ...
    captionStats.innerHTML =
      `<a class="stat-link" href="https://instagram.com/${c.instagram.handle}" target="_blank" rel="noopener">
         <span class="stat-label">Instagram</span>
         <span class="stat-count">${c.instagram.count}</span>
       </a>
       <a class="stat-link" href="https://youtube.com/${c.youtube.handle}" target="_blank" rel="noopener">
